...
Prefetches batches in advance and shuffles them to reduce correlation,
yields a numpy array of shape (batch_size, seq_len, n_views)

This create a nested iterator structure where each iterator is responsible for a specific task:
    [ [ [ [ [ (1) read document ] -> (2) sample source ] -> (3) tokenize ] ->
                (4) tokenize and build sequence of fixed seq_len ] -> (5) prefetch batches ]

Each iterator returns a tuple (output, state) where state contains all the info necessary to resume from last output.
...
from .lingua_tokenizer import TokenizerArgs, build_tokenizer
...
def tokenize(
    ...
    Yields:
    - (tokens, state) pairs, where `tokens` is a list of tokenized text,
    and `state` is the original state from the iterator.
    """
...
def get_empty_buffer_state(
    ...
    It handles cases where the rewind goes beyond the current sequence,
    adjusting the starting sequence and token index accordingly.
    """
    # We rewind n_views
...
def pack_tokens(
    ...
    This function aggregates tokens into a buffer and yields fixed-size chunks with dimensions
    ...
    Parameters:
    - iterator: An iterator that yields pairs of (tokens, state), where tokens is a 1D sequence of tokens and
    state contains all necessary information to resume iterator from current position.
    ...
    The function handles the complexity of determining the correct state for resuming iteration
    after the buffer is cleared, ensuring seamless continuation of token sequences.
    """
    buffer = []
    states = []
...
def batch_and_shuffle_prefetched_sequences(
    ...
    """
    Prepare batch in advance and shuffle them to reduce correlation inside batches
    (for ex when very long document is encountered).

    This function aggregates batches into a buffer and yields fixed-size batch size and seqlen with dimensions
    `(batch_size, seqlen, n_views)`,

    It uses a prefetch buffer to store batches in advance and shuffles them,
    the prefetch buffer is similar to `reservoir sampling`,
    but by block to preserve a smooth, easy and deterministic reloading.
    To ensure more uniform sequence sampling -> prefetch_size * batch_size * seq_len >> max_document_seqlength.

    Parameters:
    - iterator: An iterator that yields pairs of (sequence, state), where is a random sequence sampled from a corpus
    (as done by pack_tokens for example).
...
def find_and_sanitize_chunks(dataset_path: str, world_size: int) -> list[str]:
...
def distribute_data_to_rank(dataset_path: str, rank: int, world_size: int) -> JSONLState:
    """
...
def init_choice_state(
    ...
    for dataset_path in sources:
        jsonl_state = distribute_data_to_rank(os.path.join(root_dir, dataset_path), rank, world_size)
...
def feed_buffer(queue: Queue, stop_event: EventClass, iterator_builder: callable) -> None:
    """
...
@contextlib.contextmanager
def async_iterator(buffer_size: int, iterator_builder: callable) -> Iterator:
    """

...
def plot_recall_hellaswag_tv(df, save_path, dataset_size="10k", recall_threshold=0.9, save_name="triple_plot_vs_steps"):
    ...
    for i, metric in enumerate(["recall", "hellaswag_relative", "tv"]):
        ...
        y_label = {"recall": "Factual Recall", "hellaswag_relative": "HellaSwag Relative (%)", "tv": "Total Variation"}[
            metric
        ]
        ...
        ax.hlines(y=best_y, xmin=best_x, xmax=final_x, color="black", linestyle="--", linewidth=1.1)
        ...
        for model_label in sorted(df_subset["model"].unique()):
            ...
            ax.fill_between(
                sub["checkpoint_nbr"], sub[metric] - sub[yerr_col], upper, color=color_map[model_label], alpha=0.15
            )
...
def aggregate_training_steps_for_latex(df, save_path, save_name="avg_training_steps.csv"):
    ...
    tool_ckpt = in_tool_df.groupby("model").apply(lambda g: g.loc[g["checkpoint_nbr"].idxmin()]).reset_index(drop=True)

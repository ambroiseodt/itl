...
class DataCollatorForToolOnlyLM:
    ...
    def __post_init__(self):
        ...
        self.end_header_id = self.tokenizer.convert_tokens_to_ids(self.end_header_token) if self.end_header_token else None
        ...
        for i, input_ids_seq in enumerate(input_ids):
            ...
            while j < len(seq) - 1:
                if is_llama:
                    if (
                        j + 2 < len(seq) and
                        seq[j] == self.start_header_id and
                        seq[j + 1] == self.assistant_token_id and
                        seq[j + 2] == self.end_header_id
                    ):
                        start_idx = j + 3
                    else:
                        j += 1
                    ...
                    if (
                        j + len(self.assistant_token_ids) < len(seq) and
                        seq[j] == self.start_header_id and
                        seq[j + 1:j + 1 + len(self.assistant_token_ids)] == self.assistant_token_ids
                    ):
...
def inspect_collator_outputs(tokenized_dataset, collator, tokenizer, n=3):
    ...
    assert all(len(seq) == attention_mask.shape[1] for seq in attention_mask), "Not all attention_masks are padded to the same length!"
    ...
    for i in range(n):
        ...
        masked_labels = [
            tid if tid != -100 else collator.description_of_masked_label_token_id 
            for tid in labels[i].tolist()
        ]
